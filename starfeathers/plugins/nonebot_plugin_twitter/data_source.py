...
async def get_user_info(name: str, token: str):
    ...
    params = (
        (
            "variables",
            '{"screen_name":"%s","withSafetyModeUserFields":true,"withSuperFollowsUserFields":false}'
            % (name),
        ),
    )
    async with httpx.AsyncClient(proxies=config.proxy) as client:
        try:
            response = await client.get(
                "https://mobile.twitter.com/i/api/graphql/B-dCk4ph5BZ0UReWK590tw/UserByScreenName",
                headers=headers,
                params=params,
            )
        except:
...
async def get_latest_tweet(user_id, token):
    ...
    async with httpx.AsyncClient(proxies=config.proxy) as client:
        try:
            response = await client.get(
                "https://mobile.twitter.com/i/api/graphql/OMMpfG9VCdQAK0KHaU1RSQ/UserTweets",
                headers=headers,
                params=params,
            )
        except:
...
def get_tweet_details(data):
    name = data[0]["content"]["itemContent"]["tweet_results"]["result"]["core"][
        "user_results"
    ]["result"]["legacy"]["name"]
    screen_name = data[0]["content"]["itemContent"]["tweet_results"]["result"]["core"][
        "user_results"
    ]["result"]["legacy"]["screen_name"]
    quote = data[0]["content"]["itemContent"]["tweet_results"]["result"]
    data = retweet = data[0]["content"]["itemContent"]["tweet_results"]["result"][
        "legacy"
    ]
    is_quote = False
    is_retweet = False
